...
import { useCertificateListAccent, useCertificateSource, useCertificateTime } from "@/hooks";
...
const CertSearchCard = memo(({ cert }: { cert: CertificateInfo }) => {
  ...
  const accent = useCertificateListAccent(cert);
      ...
      <div className={styles.cardInner}>
        ...
        <div className={styles.certInfo}>
          ...
          {cert.sans && cert.sans.length > 0 && (
            <div className={`${styles.certInfoItem} ${styles.certInfoWide}`}>
              <span className={styles.certInfoLabel}>{t("cert.sans") || "SANs"}</span>
              <span className={styles.certInfoValue} title={cert.sans.join(", ")}>
                {cert.sans.join(", ")}
              </span>
            </div>
          )}
