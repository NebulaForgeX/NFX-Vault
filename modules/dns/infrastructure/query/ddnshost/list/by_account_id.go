...
func (h *Handler) ByAccountID(ctx context.Context, accountID uuid.UUID) ([]ddnsQuery.HostVO, error) {
	...
	if err := h.db.WithContext(ctx).Table(views.NamecheapDdnsHostsActiveView{}.TableName()).
		Where("account_id = ?", accountID).Order("domain, host").Find(&rows).Error; err != nil {
		return nil, err
	}
