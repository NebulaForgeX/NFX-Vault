...
def _reapply_manual_add_certificate_background(
    ...
    Args:
        ...
        sans: SANs 列表
        webroot: Webroot 路径
        original_cert: 原始证书信息（包含 domain, folder_name 等）
    ...
    try:
        ...
        logger.info(f"🚀 Starting MANUAL_ADD certificate reapplication in background for domain '{domain}'")
        ...
        if apply_result["success"]:
            try:
                ...
                if cert_pem and key_pem:
                    ...
                    # 只更新证书内容、私钥内容和事件信息，保持其他字段不变（包括 store）
                    ...
                    logger.info(f"✅ Certificate content and private key updated for domain '{domain}'")
                else:
                ...
                # 发布缓存失效事件
                original_store = original_cert.get("store")
                app.invalidate_cache([original_store], trigger="update")
                logger.info(f"✅ MANUAL_ADD certificate reapplication completed for domain '{domain}'")
                ...
                logger.error(f"❌ MANUAL_ADD certificate reapplication failed for domain '{domain}': {str(e)}. Status restored to original.")
        else:
            ...
            logger.error(f"❌ MANUAL_ADD certificate reapplication failed for domain '{domain}': {error_msg}. Status restored to original.")
    
    except Exception as e:
        logger.error(f"❌ MANUAL_ADD 证书重新申请异常: {e}", exc_info=True)
        try:
            # 恢复原始状态，并保存错误信息
            from datetime import datetime
            app.database_repo.update_certificate_by_id(
            ...
            logger.error(f"❌ MANUAL_ADD certificate reapplication exception for domain '{domain}': {str(e)}. Status restored to original.")
        except:
            pass
...
def reapply_manual_add_certificate(
    ...
    try:
        ...
        logger.info(f"🚀 Starting MANUAL_ADD certificate reapplication for domain '{domain}' (async)")
        ...
        thread = threading.Thread(
            target=_reapply_manual_add_certificate_background,
            args=(app, certificate_id, email, sans, webroot, original_cert, original_status, force_renewal),
            daemon=True,
            name=f"cert-reapply-manual-add-{domain}"
        )
        thread.start()
        logger.info(f"✅ MANUAL_ADD certificate reapplication started in background thread for domain '{domain}'")
        
        return {
            "success": True,
            "message": f"MANUAL_ADD certificate reapplication started for domain '{domain}'. Please check the status later.",
            "status": CertificateStatus.PROCESS.value,
            "error": None
        }
        ...
        return {
            "success": False,
            "message": f"Error starting MANUAL_ADD certificate reapplication: {str(e)}",
            "status": CertificateStatus.FAIL.value,
            "error": str(e)
        }
