...
"""
AUTO 证书重新申请 Operation
"""
import logging
from fastapi import HTTPException
from modules.applications.tls import CertificateApplication
from modules.interfaces.http.dto.reqdto.certificate import ReapplyAutoCertificateRequest
...
def reapply_auto_certificate(
    ...
    request: ReapplyAutoCertificateRequest
):
    """
    ...
    # 验证证书 source 是 AUTO
    if original_cert.get("source") != CertificateSource.AUTO.value:
        raise HTTPException(
            status_code=400, 
            detail=f"Certificate source is not AUTO, got {original_cert.get('source')}"
        )
    ...
    return app.reapply_auto_certificate(
