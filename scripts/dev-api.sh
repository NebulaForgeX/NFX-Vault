#!/usr/bin/env bash
# 仓库根目录：创建/同步 venv + 启动统一 backend（HTTP + Kafka Consumer + 调度，uvicorn 热重载）
set -euo pipefail
...
echo "[NFX-Vault] backend 端口: ${NFX_VAULT_API_PORT}（与 frontend/vite 代理一致）" >&2
exec "$VENV_PY" -m uvicorn main:app --host 0.0.0.0 --port "${NFX_VAULT_API_PORT}" --reload --log-level debug
